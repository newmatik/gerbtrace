...
description: Reusable sidebar UX pattern used in the panel editor and other screens.
---
...
1. **Global quick actions (top row)** — binary toggles and frequently used actions (e.g., component overlay, danger zone).
2. **Status strip** — compact computed info and warnings (e.g., panel size and max-limit warning).
3. **Tabbed content** — group related settings by user intent, not by implementation detail. Current pattern:
...
- Wrap logically related controls in bordered cards: `rounded border ... p-2 space-y-2`.
...
- Keep labels short, unit-explicit, and consistent: `Routing tool (mm)`, `Hole dia (mm)`, `Corner R (mm)`.
