...
pnpm run packages:tree:manifest

# Validate SMD library
pnpm run packages:tree:check

# Regenerate THT manifest
pnpm run packages:tht:manifest
```
...
pnpm run packages:migrate:newmatik
