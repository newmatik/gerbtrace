...
| `deploy-supabase.yml` | Push to `main` (changes in `supabase/**`) | Legacy, disabled — migrations are applied via Supabase MCP tools |
...
Database migrations are applied directly to **Supabase Cloud** (project `gqrnlnlfidighosujpdb`) using the Supabase MCP tools (`apply_migration`, `execute_sql`). The legacy `deploy-supabase.yml` workflow is disabled.
