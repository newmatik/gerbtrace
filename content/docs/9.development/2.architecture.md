...
Gerbtrace is built as a **hybrid Nuxt app** (SSR for marketing pages, SPA for the authenticated app) and a **Tauri desktop app** that bundles the same frontend. Collaboration features are backed by **Supabase Cloud**.

## High-level components

The system consists of three main deployment targets:

- **Web app** – hosted on Cloudflare Workers with hybrid rendering (SSR for public pages, SPA for authenticated routes)
...
| Route | Purpose |
|---|---|
| `/api/ai/enrich-bom` | Spark AI BOM enrichment (server-side credential resolution) |
| `/api/ai/validate-key` | Validate Spark AI provider key (admin use) |
| `/api/elexess/search` | Proxied Elexess search (server-side credential resolution) |
| `/api/elexess/validate-credentials` | Validate Elexess credentials (admin use) |
