...
|---|---|---|
| `PT_TWO_POLE` | Two-terminal chip-style passives (R/C/L and true chip-style diodes) | Chip (2-pole) |
...
### Molded diode families (DO-214/DO-213/DO-219/DO-221 and similar)

For molded diode packages with formed terminals, do **not** model them as `PT_TWO_POLE`
unless the package is truly chip-style.

Use `PT_GENERIC` with explicit lead groups and the correct lead shape:

- Use `J_LEAD` for J-lead style molded packages.
- Keep terminal width (`padWidth`) based on lead/terminal band dimensions (`b`), not body width.
- Keep terminal length (`padLength`) based on lead length (`L`) / wrapped-foot geometry.
- Keep package orientation in Mycronic/TPSys baseline; pin 1 marker should remain upper-left
  in the rendered 0-degree baseline.

This avoids rendering diodes as full-height chip end caps when the real package has narrower
formed terminals.
...
2. Regenerate manifests: `pnpm run packages:tree:manifest`.
3. Validate: `pnpm run packages:tree:check`.
